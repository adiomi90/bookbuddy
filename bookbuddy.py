...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            success, message, user_id = user_manager.create_user(username, email, password)
            print(message)

        elif choice == "2":
            email = input("Enter your email: ")
            password = input("Enter your password: ")
            success, message, user_id = user_manager.authenticate_user(email, password)
            print(message)
            if success:
                while True:
                    print("\nBookBuddy Menu:")
                    ...
                    if user_choice == "1":
                        ...
                        blog_manager.add_blog_post(user_id, title, content)
                        print("Blog post created successfully.")

                    elif user_choice == "3":
                        topic = input("Enter discussion topic: ")
                        content = input("Enter discussion content: ")
                        community_manager.start_discussion(user_id, topic, content)
